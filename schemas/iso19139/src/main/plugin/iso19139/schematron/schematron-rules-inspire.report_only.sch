...
-->
  ...
  <sch:pattern>
    <sch:title>$loc/strings/spatialscope</sch:title>

    <sch:rule context="//gmd:MD_DataIdentification|
			//*[@gco:isoType='gmd:MD_DataIdentification']">
      <!-- Check that INSPIRE Spatial Scope are available.
      -->

      <sch:let name="spatial-scope-thesaurusURL" value="concat('file:///', replace($thesaurusDir, '\\', '/'), '/external/thesauri/theme/SpatialScope.rdf')"/>
      <sch:let name="spatial-scope-thesaurus" value="document($spatial-scope-thesaurusURL)"/>
      <sch:let name="spatial-scope" value="$spatial-scope-thesaurus//rdf:Description"/>

      <!-- Display error if SpatialScope thesaurus is not available. -->
      <sch:assert test="count($spatial-scope) > 0">
        INSPIRE - Spatial scope thesaurus not found (at <sch:value-of select="$spatial-scope-thesaurusURL" />. Check installation in codelist/external/thesauri/theme.
        Download thesaurus from INSPIRE Registry.
      </sch:assert>
      ...
      <sch:let name="spatial-scope-found"
               value="count($spatial-scope-thesaurus//rdf:Description[skos:prefLabel = $keyword])"/>

      <sch:assert test="$spatial-scope-found > 0" see="geonet:child[@name='descriptiveKeywords']/@uuid">
        <sch:value-of select="$loc/strings/alert.M70/div"/>
      </sch:assert>

      <sch:report test="$spatial-scope-found > 0">
        <sch:value-of select="$spatial-scope-found"/> <sch:value-of select="$loc/strings/report.M70/div"/>
      </sch:report>
    ...
    <sch:pattern>
        ...
        <sch:rule context="//srv:SV_ServiceIdentification|//*[@gco:isoType='srv:SV_ServiceIdentification']">
            <!-- Check that INSPIRE service taxonomy is available.
                Use INSPIRE thesaurus available on SVN to check keywords in all EU languages.
            -->
            <sch:let name="inspire-thesaurus-path" value="replace(concat('file:///', $thesaurusDir, '/external/thesauri/theme/inspire-service-taxonomy.rdf'), '\\', '/')"/>
            ...
            <sch:let name="keywordAsAnchor"
                value="gmd:descriptiveKeywords/*/gmd:keyword/gmx:Anchor/@xlink:href"/>
            <sch:let name="inspire-theme-found"
                value="count($inspire-thesaurus//skos:Concept[skos:prefLabel = $keyword]) + count($inspire-thesaurus//skos:Concept[@rdf:about = $keywordAsAnchor])"/>
            <sch:assert test="$inspire-theme-found > 0" see="geonet:child[@name='descriptiveKeywords']/@uuid">
    ...
    <sch:pattern>
        ...
        <sch:rule context="/gmd:MD_Metadata[gmd:identificationInfo/gmd:MD_DataIdentification]">
          ...
          <sch:assert test="$lineageDefined"
                      see="gmd:dataQualityInfo/gmd:DQ_DataQuality/geonet:child[@name='lineage']/@uuid">
            <sch:value-of select="$loc/strings/alert.M43/div"/>
          </sch:assert>

          <sch:let name="statementDefined" value="gmd:dataQualityInfo/*/gmd:lineage/*/gmd:statement/node()"/>
          <sch:assert test="$statementDefined"
                      see="gmd:dataQualityInfo/*/gmd:lineage/*/geonet:child[@name='statement']/@uuid">
            <sch:value-of select="$loc/strings/alert.M43/div"/>
          </sch:assert>
          ...
          <sch:assert test="$hasAtLeastOneLineage"
                      see="gmd:dataQualityInfo/*/gmd:lineage/*/gmd:statement/geonet:element/@ref">
            <sch:value-of select="$loc/strings/alert.M43/div"/>
          </sch:assert>
    ...
    <sch:pattern>
        ...
        <sch:rule context="//gmd:MD_DataIdentification|
            ...
            <sch:report test="$accessConstraints_found">
                <sch:value-of select="$loc/strings/report.M45.ca/div"/> <sch:value-of select="$accessConstraints_count"/>
            </sch:report>
            ...
            <sch:report test="($otherConstraintCharacterStringInfo!='' or $otherConstraintAnchorInfo!='') and not($accessConstraints) and not($otherConstraints)">
    ...
    <sch:pattern>
        ...
        <sch:rule context="//gmd:MD_DataIdentification|
			...
			//srv:SV_ServiceIdentification|
			//*[@gco:isoType='srv:SV_ServiceIdentification']">
            <sch:let name="mdConstraints_count" value="count(gmd:resourceConstraints/gmd:MD_Constraints)"/>
            <sch:let name="useLimitation" value="gmd:resourceConstraints/gmd:MD_Constraints/gmd:useLimitation/*/text()"/>
            <sch:let name="useLimitation_count" value="count(gmd:resourceConstraints/gmd:MD_Constraints/gmd:useLimitation/*/text())"/>

          <sch:assert test="count(gmd:resourceConstraints/*) > 0" see="geonet:child[@name='resourceConstraints']/@uuid">
            <sch:value-of select="$loc/strings/alert.M45/div"/>
          </sch:assert>

          <sch:assert test="not($mdConstraints_count and not($useLimitation_count))" see="gmd:resourceConstraints/*/gmd:useLimitation/geonet:element/@ref">
                <sch:value-of select="$loc/strings/alert.M45.us/div"/>
            </sch:assert>

            <sch:report test="$mdConstraints_count and $useLimitation_count">
    ...
    <sch:pattern>
        ...
        <sch:rule context="//gmd:MD_Metadata">
            <!--  Date stamp -->
            <!--sch:let name="dateStamp" value="gmd:dateStamp/gco:Date/text()"/>
